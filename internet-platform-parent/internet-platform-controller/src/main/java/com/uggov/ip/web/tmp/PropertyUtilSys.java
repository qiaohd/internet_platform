...
/************工作流开关**********************/
    ...
    /************路径配置信息**********************/
    private static Properties pathProp = null;

    static {
    	pathProp = new Properties();
        InputStream in = null;
        try {
            in =new ClassPathResource("ipconfig/path.properties").getInputStream();
            pathProp.load(in);
        } catch (IOException e) {
            e.printStackTrace();
        }finally{
    ...
    public static String getPathPropertyByKey(String key) {
