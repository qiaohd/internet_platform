...
var SNSRoster=function(a,b){this.name;this.id;this.photoUrl;this.vcard;this.presence=new SNSPresence;this.groups=[];this.subscription=SNS_SUBSCRIBE.NONE;this.resource;this.ask="";a&&(this.id=a,this.name=b?b:a);this._queryingVCard=!1;this._vcardDefer;this.entityType="chat"};SNSRoster.prototype.getPhotoUrl=function(){return this.photoUrl?YYIMChat.getFileUrl(this.photoUrl):SNSConfig.USER.DEFAULT_AVATAR};SNSRoster.prototype.rename=function(a){SNSRoster.checkName(a);this.name=a;return this.update()};
...
SNSTabContainer.prototype.getContentContainer=function(){this._contentContainer&&0!=this._contentContainer.length||(this._contentContainer=jQuery(this.contentContainerSelector));return this._contentContainer};
SNSTabContainer.prototype._bindDomEvent=function(a){var b=this;a.getHeadDom().bind("click",function(){a.roster?YYIMChat.getVCard({id:a.roster.id,success:function(c){a.roster.setVCard(new SNSVCard(c));a.roster.name=c.nickname||a.roster.id;jQuery(".sns_curchat_title").html(a.roster.name);jQuery("#snsim_chat_window_roster_tab_head_"+a.roster.id).attr("title",a.roster.name).find(".snsim_username").html(a.roster.name+'<span class="wbim_icon_vf"></span>');tag.roster.photo=c.photo;b.changeTabTo(a)}}):b.changeTabTo(a)})};
SNSTabContainer.prototype.getTab=function(a){return this.tabs.get(a)};SNSTabContainer.prototype.size=function(){return this.tabs.size()};SNSTabContainer.prototype.removeTab=function(a){var b=this.getCurrentTab();this.tabs.remove(a);if(b.name==a.name&&(this.tabs.tabCurName=null,0!=this.size()))return this.tabs.toArray()[0]};SNSTabContainer.prototype.onTabChange=function(a,b){};
...
SNSChatWindow.prototype.chatWithAnyRoster=function(a){if(a&&a.id&&(this.queryRosterIng=this.queryRosterIng||{},!this.queryRosterIng[a.id])){this.queryRosterIng[a.id]=!0;var b=SNSApplication.getInstance().getUser(),c=b.getRoster(a.id);c||(c=new SNSRoster(a.id),b.addRoster(c,!0));var d=this;YYIMChat.getVCard({id:c.id,success:function(b){c.setVCard(new SNSVCard(b));c.name=b.nickname||a.id;c.photo=b.photo;c.queryed=!0;d.queryRosterIng[a.id]=!1;SNSIMWindow.getInstance().getChatWindow().openChatWithOld(a.id)}})}};
SNSChatWindow.prototype.openChatWith=function(a,b){var c;a.id?(c=a.id,b=a.name||b):c=a;if(c){var d=SNSApplication.getInstance().getUser().getRosterOrChatRoom(c);d&&"chat"==d.entityType&&"both"!=d.subscription?this.chatWithAnyRoster({id:c,name:b}):d&&(d.name!=d.id||d.queryed)?this.openChatWithOld(a):this.chatWithAnyRoster({id:c,name:b})}};
