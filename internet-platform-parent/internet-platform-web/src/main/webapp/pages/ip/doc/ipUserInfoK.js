define([ 'jquery', 'knockout','text!./ipUserInfoK.html','uui','grid','css!./ipUserInfoK.css'],
	function($, ko, template) {
			...
			for(var j=0;j<data.length;j++){
				if(data[j].showType){
					if(data[j].showType == 'dateselect'){
						...
						innerHTML += "<div class='search-common clearfix'>";
						innerHTML += "<label>"+ data[j].columnComments +":</label>";
						innerHTML += "<div id='"+ data[j].javaField + flag_name + "' class='"+ data[j].javaField +" u-combo u-text u-label-floating' u-meta='"+'{"id":"'+data[j].javaField +'","type":"u-combobox","data":"'+ flag_name +'","isAutoTip":true,"field":"'+data[j].javaField+'"}'+ "'>";
		...
		delFun = function (index) {
			...
			$.ajax({
				url: "finaltest/ipUserInfoK/delete",
				type: "POST",
 				dataType: "json",
		...
		viewModel.searchInfo = function () {
			...
			console.log(datas);
			$.ajax({
				url: "finaltest/ipUserInfoK/list",
				type: "POST",
				dataType: "json",
				data: datas,
				success: function (data) {
		...
		viewModel.saveInfoEdit = function () { 
			...
			console.log(datas);
			$.ajax({
				url: "finaltest/ipUserInfoK/update",
				type: "POST",
				dataType: "json",
				data: datas,
				success: function (data) {
		...
		viewModel.saveInfoAdd = function () {
			...
			console.log(datas);
			$.ajax({
				url: "finaltest/ipUserInfoK/save",
				type: "POST",
				dataType: "json",
				data: datas,
				success: function (data) {
