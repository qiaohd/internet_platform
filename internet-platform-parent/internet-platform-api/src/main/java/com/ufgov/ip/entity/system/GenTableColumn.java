...
public class GenTableColumn extends DataEntity<GenTableColumn> {
	...
	private String columnComments="";	// 描述
	...
	public String getNameAndComments() {
		return getColumnName() + (columnComments == null ? "" : "  :  " + columnComments);
	}
	...
	public List<String> getAnnotationList(){
		...
		if (!"1".equals(getIsNull()) && !"String".equals(getJavaType())){
			list.add("javax.validation.constraints.NotNull(message=\""+getColumnComments()+"不能为空\")");
		}
			...
			list.add("org.hibernate.validator.constraints.Length(min=1, max="+getDataLength()
					+", message=\""+getColumnComments()+"长度必须介于 1 和 "+getDataLength()+" 之间\")");
		}
			...
			list.add("org.hibernate.validator.constraints.Length(min=0, max="+getDataLength()
					+", message=\""+getColumnComments()+"长度必须介于 0 和 "+getDataLength()+" 之间\")");
		}
		return list;
	}
